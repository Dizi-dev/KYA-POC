...
from kya_gateway.keys import KNOWN_TEST_KEYIDS, WELL_KNOWN, KeyResolver, jwk_thumbprint
...
def survey(origin: str) -> dict:
    ...
    row = {"origin": origin, "url": url, "checked_at": dt.datetime.now(dt.timezone.utc).isoformat()}
    try:
        r = httpx.get(url, timeout=10, follow_redirects=False, headers={"accept": "application/json"})
...
def main():
    ...
    args = ap.parse_args()
    results = []
    for o in (args.origins or CANDIDATES):
        row = survey(o)
        results.append(row)
